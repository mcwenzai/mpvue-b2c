      ...
      <view class="padding">暂无收藏～</view>
		...
		<view class="cu-list menu-avatar" v-else>
			<view v-for="(item,index) in goodsList" :key="index" class="cu-item" :class="modalName=='move-box-'+ index?'move-cur':''"  @touchstart="ListTouchStart" @touchmove="ListTouchMove" @touchend="ListTouchEnd" :data-target="'move-box-'+index">
				...
				<view class="move">
					<view class="bg-grey" @click="goUrl(item.id,item.productId,index)">去店铺</view>
	...
	export default {
		...
		methods: {
			...
			getInfo(){
				...
				this.$fly.post('/product/collection/query',{"offset":0,"limit":10}).then(res => {
          ...
          this.goodsList = []
				...
				this.$fly.post('/product/collection/query',params).then(res => {
          ...
          this.downIndex++;
			...
			deleted(id,index){
				...
				var that = this;
				this.$fly.post('/product/collection/delete',{"productCollectionId":[id]}).then(res => {
					wx.showToast({
						title: '删除成功',
						success: function(){
